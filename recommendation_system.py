...
db_url = "postgresql://postgres:1234@localhost:5432/GP"

# Create a database connection
engine = create_engine(db_url)

# Load data from PostgreSQL database
interactions_df = pd.read_sql_query('SELECT user_id, recipe_id, rating FROM interactions_table', engine)
recipes_df = pd.read_sql_query('SELECT recipe_name, recipe_id, minutes, contributor_id, submitted, tags, nutrition, n_steps, steps, description, ingredients, n_ingredients, price FROM recipes_table', engine)

# Rename columns for clarity
recipes_df.columns = ['recipe_name', 'recipe_id', 'minutes', 'contributor_id', 'submitted', 'tags', 'nutrition', 'n_steps', 'steps', 'description', 'ingredients', 'n_ingredients', 'price']
